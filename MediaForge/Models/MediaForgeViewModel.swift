...
/// ViewModel for the MediaForge application
class MediaForgeViewModel: ObservableObject {
    ...
    // MARK: - Quality Check Features
    
    /// Quality check preview settings
    @Published var enableQualityCheckFeatures: Bool = false
    @Published var showWaveform: Bool = false
    @Published var showVectorscope: Bool = false
    @Published var showHistogram: Bool = false
    @Published var showFrameLines: Bool = false
    @Published var frameLineAspectRatio: String = "2.39:1"
    @Published var frameLinesAppearance: FrameLinesAppearance = .solid
    
    /// Available frame line appearance options
    enum FrameLinesAppearance: String, CaseIterable, Identifiable {
        case solid = "Solid"
        case dashed = "Dashed"
    ...
    /// Custom frame line aspect ratios that user has saved
    @Published var savedFrameLineAspectRatios: [String] = [
        "1.85:1 (Widescreen)",
        "2.39:1 (Cinemascope)",
        "16:9 (HD)",
        "4:3 (SD)"
    ...
    /// Add a custom frame line aspect ratio
    func addFrameLineAspectRatio(_ ratio: String) {
        guard !savedFrameLineAspectRatios.contains(ratio) else { return }
        savedFrameLineAspectRatios.append(ratio)
        savedFrameLineAspectRatios.sort()
    }
    
    // MARK: - Media Preview & Clip Review
    
    /// Current clip being reviewed
    @Published var currentPreviewClip: URL?
    
    /// Image transposition settings for preview
    @Published var flipHorizontal: Bool = false
    @Published var flipVertical: Bool = false
    @Published var desqueezeRatio: Double = 1.0
    @Published var desqueezeEnabled: Bool = false
    
    /// Take a still image from current preview
    func takeStillImage(fromURL url: URL, atTime seconds: Double, completion: @escaping (Result<URL, Error>) -> Void) {
        // Implementation will extract a frame from video at the specified time
        // and save it as a still image
        
        let destinationFolder = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Documents")
            .appendingPathComponent("MediaForge Stills")
        
        do {
            try FileManager.default.createDirectory(at: destinationFolder, withIntermediateDirectories: true)
            
            // Here we would use AVFoundation to extract the frame
            // For now this is a placeholder
            
            let outputURL = destinationFolder.appendingPathComponent("\(UUID().uuidString).jpg")
    ...
    /// Configure playback for professional monitoring
    func configureExternalOutput(enabled: Bool) {
        // This would configure external video output like SDI or HDMI
    ...
    init() {
        ...
        // Create default transcoding profiles if none exist
        if transcodingProfiles.isEmpty {
            transcodingProfiles = [
                TranscodingProfile.createProxyProfile(),
                TranscodingProfile.createDailiesProfile(),
                TranscodingProfile.createArchivalProfile()
            ]
            saveTranscodingProfiles()
        }
    }
    ...
    @Published var customMetadataFields: [MetadataField] = []
    
    /// Media catalog items
    @Published var catalogItems: [CatalogItem] = []
    
    /// Media collections
    @Published var mediaCollections: [MediaCollection] = []
    
    /// Add a new custom metadata field
    func addMetadataField(_ field: MetadataField) {
        // Check for duplicate names
        if !customMetadataFields.contains(where: { $0.name == field.name }) {
            customMetadataFields.append(field)
            saveMetadataFields()
        }
    }
    
    /// Remove a custom metadata field
    func removeMetadataField(_ field: MetadataField) {
        customMetadataFields.removeAll(where: { $0.id == field.id })
        saveMetadataFields()
    }
    
    /// Update an existing metadata field
    func updateMetadataField(_ field: MetadataField) {
        if let index = customMetadataFields.firstIndex(where: { $0.id == field.id }) {
            customMetadataFields[index] = field
            saveMetadataFields()
        }
    }
    
    /// Save custom metadata fields to storage
    private func saveMetadataFields() {
        do {
            let encoder = JSONEncoder()
            let data = try encoder.encode(customMetadataFields)
            UserDefaults.standard.set(data, forKey: "CustomMetadataFields")
        } catch {
            print("Error saving custom metadata fields: \(error)")
        }
    }
    
    /// Load custom metadata fields from storage
    private func loadMetadataFields() {
        if let data = UserDefaults.standard.data(forKey: "CustomMetadataFields") {
            do {
                let decoder = JSONDecoder()
                customMetadataFields = try decoder.decode([MetadataField].self, from: data)
            } catch {
                print("Error loading custom metadata fields: \(error)")
            }
        } else {
            // Create default metadata fields if none exist
            customMetadataFields = createDefaultMetadataFields()
            saveMetadataFields()
        }
    }
    
    /// Create default metadata fields
    private func createDefaultMetadataFields() -> [MetadataField] {
        return [
            MetadataField(name: "Scene", type: .text),
            MetadataField(name: "Take", type: .number),
            MetadataField(name: "Location", type: .text),
            MetadataField(name: "Camera", type: .text),
            MetadataField(name: "Shoot Date", type: .date),
            MetadataField(name: "Director", type: .text),
            MetadataField(name: "DOP", type: .text),
            MetadataField(name: "Production", type: .text),
            MetadataField(name: "Status", type: .dropdown, options: ["Approved", "Rejected", "Review", "Backup Only"])
        ]
    }
    
    /// Add a new catalog item
    func addCatalogItem(_ item: CatalogItem) {
        catalogItems.append(item)
        saveCatalogItems()
    }
    
    /// Remove a catalog item
    func removeCatalogItem(_ item: CatalogItem) {
        catalogItems.removeAll(where: { $0.id == item.id })
        saveCatalogItems()
    }
    
    /// Update a catalog item
    func updateCatalogItem(_ item: CatalogItem) {
    ...
    /// Save catalog items to storage
    private func saveCatalogItems() {
        do {
            let encoder = JSONEncoder()
            let data = try encoder.encode(catalogItems)
            UserDefaults.standard.set(data, forKey: "CatalogItems")
        } catch {
            print("Error saving catalog items: \(error)")
        }
    }
    ...
    private func loadCatalogItems() {
        if let data = UserDefaults.standard.data(forKey: "CatalogItems") {
            do {
                let decoder = JSONDecoder()
                catalogItems = try decoder.decode([CatalogItem].self, from: data)
            } catch {
    ...
    func addMediaCollection(_ collection: MediaCollection) {
        mediaCollections.append(collection)
        saveMediaCollections()
    }
    
    /// Remove a media collection
    func removeMediaCollection(_ collection: MediaCollection) {
        mediaCollections.removeAll(where: { $0.id == collection.id })
        saveMediaCollections()
    }
    
    /// Update a media collection
    func updateMediaCollection(_ collection: MediaCollection) {
        if let index = mediaCollections.firstIndex(where: { $0.id == collection.id }) {
            mediaCollections[index] = collection
            saveMediaCollections()
        }
    }
    
    /// Save media collections to storage
    private func saveMediaCollections() {
        do {
            let encoder = JSONEncoder()
            let data = try encoder.encode(mediaCollections)
            UserDefaults.standard.set(data, forKey: "MediaCollections")
        } catch {
            print("Error saving media collections: \(error)")
        }
    }
    
    /// Load media collections from storage
    private func loadMediaCollections() {
        if let data = UserDefaults.standard.data(forKey: "MediaCollections") {
            do {
                let decoder = JSONDecoder()
                mediaCollections = try decoder.decode([MediaCollection].self, from: data)
            } catch {
                print("Error loading media collections: \(error)")
            }
        }
    }
    
    // MARK: - Media Catalog Actions
    
    /// Export metadata for selected catalog items to various formats
    func exportMetadata(for items: [CatalogItem], format: String, outputPath: String, completion: @escaping (Result<URL, Error>) -> Void) {
        // Implementation would vary based on format
        // This is a placeholder that would be implemented with actual export logic
        // for formats like ALE, XML, CSV, etc.
        
        let outputURL = URL(fileURLWithPath: outputPath)
        
        switch format {
        case "XML":
            // Generate XML for NLEs like Premiere Pro
            break
        case "ALE":
            // Generate ALE for Avid Media Composer
            break
        case "CSV":
            // Generate CSV for general use
            break
        default:
            break
        }
    ...
    func extractMetadataFromMedia(path: String, completion: @escaping (Result<[String: String], Error>) -> Void) {
        // This would use AVFoundation and other framework APIs to
    ...
    /// Transcoding profiles for different output types
    @Published var transcodingProfiles: [TranscodingProfile] = []
    
    /// Active transcoding profile
    @Published var activeTranscodingProfile: TranscodingProfile?
    
    /// Add a new transcoding profile
    func addTranscodingProfile(_ profile: TranscodingProfile) {
        transcodingProfiles.append(profile)
        saveTranscodingProfiles()
    }
    
    /// Remove a transcoding profile
    func removeTranscodingProfile(_ profile: TranscodingProfile) {
        transcodingProfiles.removeAll(where: { $0.id == profile.id })
        
        // If the active profile was removed, clear it
        if activeTranscodingProfile?.id == profile.id {
            activeTranscodingProfile = nil
        }
    ...
    func updateTranscodingProfile(_ profile: TranscodingProfile) {
        if let index = transcodingProfiles.firstIndex(where: { $0.id == profile.id }) {
            transcodingProfiles[index] = profile
            
            // Update active profile if it was the one being edited
            if activeTranscodingProfile?.id == profile.id {
                activeTranscodingProfile = profile
            }
    ...
    /// Save transcoding profiles to storage
    private func saveTranscodingProfiles() {
        do {
            let encoder = JSONEncoder()
            let data = try encoder.encode(transcodingProfiles)
            UserDefaults.standard.set(data, forKey: "TranscodingProfiles")
        } catch {
            print("Error saving transcoding profiles: \(error)")
        }
    }
    
    /// Load transcoding profiles from storage
    private func loadTranscodingProfiles() {
        if let data = UserDefaults.standard.data(forKey: "TranscodingProfiles") {
            do {
                let decoder = JSONDecoder()
                transcodingProfiles = try decoder.decode([TranscodingProfile].self, from: data)
            } catch {
                print("Error loading transcoding profiles: \(error)")
            }
        }
    }
    
    /// Transcode a media file using the current profile
    func transcodeMedia(inputURL: URL, outputPath: String, profile: TranscodingProfile, progressHandler: @escaping (Double) -> Void, completionHandler: @escaping (Result<URL, Error>) -> Void) {
        // This would be implemented with AVFoundation for actual transcoding
